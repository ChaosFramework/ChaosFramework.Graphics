...
namespace ChaosFramework.Graphics.Imaging.Formats
{
    ...
    public static class Png
    {
            ...
            for (int chunkIndex = 0; ; ++chunkIndex)
            {
                ...
                switch (type)
                {
                    case Chunks.IHDR:
                        ...
                        {
                            width = ReadUInt32BE(chunkRd);
                            height = ReadUInt32BE(chunkRd);
        ...
        static uint ReadUInt32BE(BinaryReader br)
            => (uint)((br.ReadByte() << 24) | (br.ReadByte() << 16) | (br.ReadByte() << 8) | br.ReadByte());
